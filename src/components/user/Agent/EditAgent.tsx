...
import { updateAgentData, getRegionUser } from '../../../utils/user/agent/DataAgent'; 
...
const EditAgent: React.FC = () => {
  ...
  useEffect(() => {
    if (!user) {
      navigate('/dashboard/user/agents'); // Redirect if no user is found
    }else{
  ...
  const handleSave = async () => {
    ...
    try {
      await updateAgentData(user.id,updatedAgent);
      navigate(-1); 
    } catch (error) {
  ...
  return (
    <Box sx={{ mt: 2, mb: 2, p: 2 }}>
      <Typography variant="h4" component="h2">Update Agent</Typography>
